...
1. Products × Suppliers (Cross Join)
...
2. Departments × Employees (Cross Join)
...
3. Products supplied by Suppliers (INNER JOIN)
...
10. Orders × Products where price > 100
...
11. Mismatched employee-department
...
12. Ordered quantity > stock quantity
...
13. Customers × Sales (Orders) with amount ≥ 500

SELECT C.FirstName + ' ' + C.LastName AS CustomerName, O.ProductID, O.TotalAmount
FROM Customers C
INNER JOIN Orders O ON C.CustomerID = O.CustomerID
WHERE O.TotalAmount >= 500;


14. Students × Enrollments × Courses
...
16. Orders × Payments (payment < total)
...
18. Products × Categories (Electronics / Furniture)
...
19. Sales × Customers (USA)

SELECT O.OrderID, C.FirstName, C.LastName, C.Country
FROM Orders O
INNER JOIN Customers C ON O.CustomerID = C.CustomerID
WHERE C.Country = 'USA';


20. German customers with orders > 100
...
22. Payments not equal to Quantity × Price
...
24. Managers with lower/equal salary than subordinates
...
25. Customers with orders but no payments
