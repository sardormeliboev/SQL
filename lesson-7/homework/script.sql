...
* LESSON 7 HOMEWORK TASKS – AGGREGATE FUNCTIONS + HAVING */

/* -------------------------------
   TABLES CREATION + SAMPLE DATA
---------------------------------*/
...
-- 7. Number of employees per department
...
-- 15. AVG salary + COUNT employees per department
...
-- 22. Total & average salary by dept, avg > 65000
SELECT DepartmentName, SUM(Salary) AS TotalSalary, AVG(Salary) AS AvgSalary
FROM Employees
GROUP BY DepartmentName
HAVING AVG(Salary) > 65000;

-- 23. Orders total amount (freight > 50) + least purchases
-- (Requires tsql2012 db → not included here)

-- 25. Total sales + count unique products per month-year, filter months with ≥2 products
...
-- 26. MIN & MAX order quantity per year
