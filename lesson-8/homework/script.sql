...
-- 7. Stock label (Low Stock if <30 else Sufficient)
SELECT ProductName,
...
-- 10. Customers who placed orders in Jan 2023 but no invoices
...
-- 11. Combine all product names (with duplicates) from Products + Products_Discounted
...
-- 15. Pivot: Year column as separate columns → Population_Each_Year
...
-- 18. Pivot: City as separate columns → Population_Each_City
...
-- 20. Unpivot back Population_Each_Year → City_Population
...
-- 21. Products and number of times sold (JOIN with Sales)
SELECT p.ProductName, COUNT(s.SaleID) AS TimesSold
FROM Products p
JOIN Sales s ON p.ProductID = s.ProductID
GROUP BY p.ProductName;

-- 22. Unpivot back Population_Each_City → City_Population
