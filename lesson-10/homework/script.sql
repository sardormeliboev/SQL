...
###Lesson 10 — Joins with SQL Solutions
  
1. Employees × Departments (salary > 50000)
SELECT E.Name, E.Salary, D.DepartmentName
FROM Employees E
INNER JOIN Departments D ON E.DepartmentID = D.DepartmentID
WHERE E.Salary > 50000;

2. Customers × Orders (2023-yilda)
SELECT C.FirstName, C.LastName, O.OrderDate
FROM Customers C
INNER JOIN Orders O ON C.CustomerID = O.CustomerID
WHERE YEAR(O.OrderDate) = 2023;

3. Barcha employees va bo‘limlari (hatto bo‘limsizlari ham)
SELECT E.Name, D.DepartmentName
FROM Employees E
LEFT JOIN Departments D ON E.DepartmentID = D.DepartmentID;

4. Barcha suppliers va mahsulotlari (hatto mahsulotsizlari ham)
SELECT S.SupplierName, P.ProductName
FROM Suppliers S
LEFT JOIN Products P ON S.SupplierID = P.SupplierID;

5. Orders × Payments (hamma buyurtmalar, mos kelmaganlar ham)
SELECT O.OrderID, P.PaymentID, P.Amount
FROM Orders O
LEFT JOIN Payments P ON O.OrderID = P.OrderID;

6. Employee va uning manageri
SELECT E.Name AS EmployeeName, M.Name AS ManagerName
FROM Employees E
LEFT JOIN Employees M ON E.ManagerID = M.EmployeeID;

7. Students × Courses ("Math 101" talabalari)
...
8. Customers × Orders (3+ mahsulot olganlar)
...
11. Hech sotilmagan mahsulotlar
SELECT P.ProductName
FROM Products P
LEFT JOIN Orders O ON P.ProductID = O.ProductID
WHERE O.ProductID IS NULL;

12. Buyurtma qilgan mijozlar (soni bilan)
SELECT C.FirstName, C.LastName, COUNT(O.OrderID) AS OrdersCount
FROM Customers C
INNER JOIN Orders O ON C.CustomerID = O.CustomerID
GROUP BY C.FirstName, C.LastName;

13. Employees × Departments (NULLsiz)
SELECT E.Name, D.DepartmentName
FROM Employees E
INNER JOIN Departments D ON E.DepartmentID = D.DepartmentID;

14. Bir xil managerga bo‘ysunadigan xodimlar juftliklari
...
15. 2022-yildagi buyurtmalar
SELECT O.OrderID, O.OrderDate, C.FirstName, C.LastName
FROM Orders O
INNER JOIN Customers C ON O.CustomerID = C.CustomerID
WHERE YEAR(O.OrderDate) = 2022;

16. "Sales" bo‘limi × Employees (Salary > 60000)
SELECT E.Name, E.Salary, D.DepartmentName
FROM Employees E
INNER JOIN Departments D ON E.DepartmentID = D.DepartmentID
WHERE D.DepartmentName = 'Sales' AND E.Salary > 60000;

17. Faqat to‘lovi bor buyurtmalar
SELECT O.OrderID, P.Amount
FROM Orders O
INNER JOIN Payments P ON O.OrderID = P.OrderID;

18. Hech buyurtma qilinmagan mahsulotlar
SELECT P.ProductName
FROM Products P
LEFT JOIN Orders O ON P.ProductID = O.ProductID
WHERE O.OrderID IS NULL;

19. O‘z bo‘limidagi o‘rtacha oylikdan yuqori oladigan xodimlar
...
20. 2020-yilgacha buyurtma qilingan, ammo to‘lov qilinmagan buyurtmalar
SELECT O.OrderID, O.OrderDate
FROM Orders O
LEFT JOIN Payments P ON O.OrderID = P.OrderID
WHERE P.OrderID IS NULL AND O.OrderDate < '2020-01-01';

21. Kategoriyasi yo‘q mahsulotlar
SELECT P.ProductName
FROM Products P
LEFT JOIN Categories C ON P.Category = C.CategoryID
WHERE C.CategoryID IS NULL;

22. Bir xil managerga bo‘ysunadigan va 60000+ oylik oladigan xodimlar
SELECT E.Name, E.Salary, M.Name AS Manager
FROM Employees E
INNER JOIN Employees M ON E.ManagerID = M.EmployeeID
WHERE E.Salary > 60000;

23. Nomi "M" harfi bilan boshlanadigan bo‘lim xodimlari
SELECT E.Name, D.DepartmentName
FROM Employees E
INNER JOIN Departments D ON E.DepartmentID = D.DepartmentID
WHERE D.DepartmentName LIKE 'M%';

24. Summasi 500+ bo‘lgan sotuvlar
SELECT S.SaleID, P.ProductName, S.TotalAmount
FROM Sales S
INNER JOIN Products P ON S.ProductID = P.ProductID
WHERE S.TotalAmount > 500;

25. "Math 101" kursiga yozilmagan talabalar
SELECT S.Name
FROM Students S
...
26. To‘lov qilinmagan buyurtmalar
SELECT O.OrderID
FROM Orders O
LEFT JOIN Payments P ON O.OrderID = P.OrderID
